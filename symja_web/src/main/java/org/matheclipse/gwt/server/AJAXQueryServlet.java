...
public class AJAXQueryServlet extends HttpServlet {
  ...
  public static volatile ArrayList<String[]> INPUT_DATA = new ArrayList<String[]>();
  ...
  private String[] calculateString(EvalEngine engine, final String inputString,
    ...
    try {
      ...
      try {
        result = task.get(Config.SERVER_REQUEST_TIMEOUT_SECONDS * 1000, TimeUnit.MILLISECONDS);
  ...
  public String evaluate(HttpServletRequest request, String expression, String numericMode,
    ...
    try {
      if (userService.isUserLoggedIn()) {
        ...
        LOGGER.warn("(" + user.getEmail() + ") In::" + expression);
      ...
      if (userService.isUserLoggedIn()) {
        ...
        if (!putEntity(user, engine)) {
          // TODO error message
          return JSONBuilder
              .createJSONError("User data limit: " + HALF_MEGA + " bytes exceeded")[1];
        }
      } else {
        if (!putEntity(session, engine)) {
          // TODO error message
          return JSONBuilder
              .createJSONError("Session data limit: " + HALF_MEGA + " bytes exceeded")[1];
        }
      }
      if (outs != null) {
        outs.close();
  ...
  private static EvalEngine getEntity(User user, EvalEngine engine) {
    ...
    try {
      ...
      // rex.printStackTrace();
      LOGGER.warn("getEntity::ioexception 1");
    }
    return engine;
  }
  ...
  private static EvalEngine getEntity(HttpSession session, EvalEngine engine) {
    if (session != null) {
      ...
      try {
        ...
        rex.printStackTrace();
        LOGGER.warn("getEntity::ioexception 2");
      } finally {
        try {
          if (ois != null) {
            ois.close();
          ...
          LOGGER.fatal("getEntity::stream close problem");
        }
      }
    }
    return engine;
  ...
  private static boolean putEntity(User user, EvalEngine engine) {
    ...
    Serializable context = engine.getContextPath().getGlobalContext();
    ...
    try {
      ...
      // ex.printStackTrace();
      LOGGER.warn("putEntity::ioexception 3");
      return false;
    }
  ...
  private static boolean putEntity(HttpSession session, EvalEngine engine) {
    if (session != null) {
      ...
      try {
        ...
        ex.printStackTrace();
        LOGGER.warn("putEntity::ioexception 4");
        return false;
      } finally {
        try {
          if (oos != null) {
    ...
    try (PrintStream outs = new PrintStream(wouts);
      ...
      if (inExpr != null) {
        ...
        if (outExpr != null) {
          ...
          if (numberOfLeaves > 5 && inExpr.head() instanceof IBuiltInSymbol) {
            addMailEntry(inExpr, outExpr);
          }
  ...
  public static synchronized void initialization(String servlet) {
    ...
    Config.MAX_BIT_LENGTH = (Short.MAX_VALUE) * 8;
  ...
  private synchronized static void addMailEntry(IExpr inExpr, IExpr outExpr) {
    String textInStr = inExpr.toString();
    String textOutStr = outExpr.toString();
    if (textInStr.length() > 256) {
      textInStr = textInStr.substring(0, 256);
    }
    if (textOutStr.length() > 2048) {
      textOutStr = textOutStr.substring(0, 2048);
    }
    INPUT_DATA.add(new String[] {textInStr, textOutStr});
  }

  private synchronized static void sendMail() {
    if (INPUT_DATA.size() > 10) {
      ...
      try {
        ...
        msg.setFrom(new InternetAddress("anything@symjaweb.appspotmail.com", "Symjaweb Admin"));
        msg.addRecipient(javax.mail.Message.RecipientType.TO,
            new InternetAddress("axelclk@gmail.com", "AxKr"));
        String[] inOutStr = INPUT_DATA.get(0);
        msg.setSubject(inOutStr[0]);
        StringBuilder buf = new StringBuilder();
        buf.append(">> " + inOutStr[0] + "\n\n" + inOutStr[1] + "\n\n");
        for (int i = 0; i < INPUT_DATA.size(); i++) {
          inOutStr = INPUT_DATA.get(i);
          buf.append(">> " + inOutStr[0] + "\n\n" + inOutStr[1] + "\n\n");
        }
