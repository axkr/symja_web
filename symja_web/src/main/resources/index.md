...
Insert a keyword and append a `*` to search for keywords in the search field. 
...
For example: the upper-case identifiers `D, E, I, N`, are different from the identifiers `d, e, i, n`, whereas the 
functions like `Factorial` or `Integrate` can be entered as `factorial(100)` or `integrate(sin(x),x)`. 
If you type `SIN(x)` or `sin(x)`, Symja assumes you always mean the same built-in `Sin` function.  
...
In the console available keywords can be determined with the `?` operator
...
Documentation can be displayed by asking for information for the (case sensitive) function name.
